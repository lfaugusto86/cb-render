...
function logoGroup(x, y, targetHexH, digital) {
  if (digital) {
    // Lockup OFICIAL CB Digital = public/logo/cb-digital-logo-white.svg (réplica del componente CbDigitalLockup.astro). viewBox 300x56. VERBATIM, no redibujar.
    const s = (targetHexH * 0.80) / 34;  // el hexágono del lockup mide ~34u; lo igualamos visualmente al de CB Ventures
    return `<g transform="translate(${x},${y - 9 * s}) scale(${s})">
      <g transform="translate(2,9)">
        <polygon points="19,2 36,11.5 36,26.5 19,36 2,26.5 2,11.5" fill="none" stroke="#2DD4BF" stroke-width="1.2"/>
        <polygon points="19,8 30,14 30,24 19,30 8,24 8,14" fill="rgba(45,212,191,0.08)" stroke="#2DD4BF" stroke-width="0.6"/>
        <text x="19" y="23" text-anchor="middle" font-family="Playfair Display" font-size="11" font-weight="900" fill="#5EEAD4">CB</text>
      </g>
      <text x="50" y="29" font-family="Playfair Display" font-size="20" font-weight="900" letter-spacing="0.2" fill="#EEF0F8">CB<tspan fill="#2DD4BF">Digital</tspan></text>
      <text x="51" y="45" font-family="DM Mono" font-size="10.5" letter-spacing="1.5" fill="#2DD4BF">IA APLICADA AL NEGOCIO</text>
    </g>`;
  }
  // CB Ventures: logo oficial horizontal (CB blanco + Ventures azul, serif Gelasio).
  ...
  return `<g transform="translate(${tx},${ty}) scale(${s})">
    ...
    <text x="468" y="226" font-family="Gelasio" font-size="150" font-weight="700" dominant-baseline="middle" letter-spacing="-2"><tspan fill="#FFFFFF">CB</tspan><tspan fill="#4F6EF7">Ventures</tspan></text>
  </g>`;
}
...
export function buildSVG(o) {
  ...
  const digitalBrand = (o.variant || 'cbdigital') === 'cbdigital';
  const eyebrow = (o.eyebrow || v.eyebrow), cta = o.cta || v.cta;
  const site = o.site || (digitalBrand ? 'by CB Ventures · cbventures.eu' : 'cbventures.eu');
  ...
  ${digitalBrand ? '' : `<text x="${W - pad}" y="${logoTopY + logoHexH + 22}" text-anchor="end" font-family="DM Mono" font-size="18" letter-spacing="3" fill="${ACCENT}">${esc(eyebrow.toUpperCase())}</text>`}
